    ...
    try:
        ...
        for chunk in model.stream(messages):
            if chunk.content:
                ...
                yield json.dumps({"type": "stream", "data": chunk.content}) + "\n"
        # =========================
        # end
        # =========================
        yield json.dumps(
            {"type": "end", "data": {"answer": full_response, "sources": sources}}
        ) + "\n"
        # =========================
        # memory
        # =========================
        memory.save_context({"input": user_input}, {"output": full_response.strip()})
